...
function run(input) {
    const sum = input.map(row => {
        let division;
        row.forEach((column, i) => {
            column = Number(column);                     
            row.forEach((innerColumn, k) => {
                innerColumn = Number(innerColumn);
                if(i !== k) {
                    const innerDivision = column / innerColumn;
                    if (innerDivision % 1 === 0) {
